...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if ($id <= 0 || $batchId <= 0) {
        send_json_response([
            'success' => false,
            'message' => 'Evaluation ID and batch ID are required.',
        ], 400);
    }

    $stmt = $mysqli->prepare('DELETE FROM tmis_evaluations WHERE id = ? AND batch_id = ?');
    $stmt->bind_param('ii', $id, $batchId);
